...
console.log(`📝 Loaded ${keywords.length} keywords from editor/keywords.json`);
...
function validateAndConcatenateExamples() {
  ...
  if (exampleFiles.length === 0) {
    console.warn('⚠️  No .slang files found in examples directory');
    return { content: '', files: [] };
  }
  
  console.log(`\n📚 Found ${exampleFiles.length} example file(s): ${exampleFiles.join(', ')}`);
  ...
  if (hasErrors) {
    console.error('\n❌ Build failed: One or more example files failed validation');
    console.error('   Please fix the errors above or remove invalid examples');
    process.exit(1);
  }
  
  console.log(`\n✅ All ${validatedFiles.length} example file(s) validated successfully`);
...
console.log(`✅ Generated examples.md from ${validatedFiles.length} validated example file(s)`);
...
console.log('✅ Generated HTML with keywords');
